...
func (p *GT06Protocol) parsePacket(reader *bufio.Reader) (packet *Packet, err error) {
	...
	if err != nil {
		logger.Sugar().Errorf("parse packet Failed to peek data: %v", err)
		return nil, err
	}
	logger.Sugar().Infof("parse packet Peeked data: %x", peekData)
...
func (p *GT06Protocol) parsePacketInformation(reader *bufio.Reader, messageType MessageType) (interface{}, error) {
	logger.Sugar().Info("we are here in ParsePacketInformation")
...
func (p *GT06Protocol) parsePositioningData(reader *bufio.Reader) (positionInfo interface{}, err error) {
	logger.Sugar().Info("parsePositioningData we are here")
	defer func() {
		if r := recover(); r != nil {
			err = r.(error)
			if err != io.EOF {
